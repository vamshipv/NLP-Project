...
"""
def generate_summary_stream(user_query, aspects):
    global retrieved
    ...
    summary = generator.generate_summary(user_query, retrieved, aspects)
    output = ""
    for char in summary:
        output += char
        yield output
...
""") as demo:
    ...
    def retrieve_and_summarize(user_query, aspects):
        global retrieved
        retrieved = retriever.retrieve(user_query)
        aspects_list = [a.strip() for a in aspects.split(",")]  # Convert comma-separated string to list
        return generate_summary_stream(user_query, aspects_list)

    with gr.Row(elem_classes="centered-buttons"):  # Restore the gr.Row block
        generate_button = gr.Button("Summarize")
        show_chunks_button = gr.Button("Show Reviews")

    generate_button.click(retrieve_and_summarize, inputs=[query_input], outputs=output_text)
